package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketplaceConfirmationDialog implements IGetImageListener
   {
      ...
      public function showConfirmation(param1:int, param2:MarketPlaceOfferData) : void
      {
         ...
         this.var_81 = param2;
         if(!this.var_687 || !this._catalog || !this._catalog.localization)
         {
            return;
         }
         if(this._window)
         ...
         if(_loc3_)
         {
            if(param1 == this.var_687.const_1762)
            {
               _loc3_.text = "${catalog.marketplace.confirm_header}";
            }
            if(param1 == this.var_687.const_1681)
            {
         ...
         if(_loc3_)
         {
            _loc3_.text = "${" + this.var_687.getNameLocalizationKey(param2) + "}";
         }
         ...
         if(_loc3_)
         {
            ...
            _loc4_ = _loc4_.replace("%price%",this.var_81.price);
            _loc3_.text = _loc4_;
         }
         ...
         if(_loc3_)
         {
            ...
            {
               _loc4_ = _loc5_.raw;
               _loc4_ = _loc4_.replace("%days%",this.var_687.averagePricePeriod.toString());
               _loc6_ = this.var_81.averagePrice == 0 ? " - " : this.var_81.averagePrice.toString();
         ...
         if(_loc3_)
         {
            ...
            {
               _loc4_ = _loc5_.raw;
               _loc4_ = _loc4_.replace("%count%",this.var_81.offerCount);
               _loc3_.text = _loc4_;
            }
            else
            {
      ...
      private function setImage() : void
      {
         var _loc1_:* = null;
         var _loc2_:* = null;
         if(!this.var_81 || !this._window || !this._roomEngine)
         {
            return;
         }
         if(!this.var_81.image)
         {
            if(this.var_81.furniType == 1)
            {
               _loc1_ = this._roomEngine.getFurnitureIcon(this.var_81.furniId,this);
            }
            else if(this.var_81.furniType == 2)
            {
               _loc1_ = this._roomEngine.getWallItemIcon(this.var_81.furniId,this);
            }
            if(_loc1_ && _loc1_.data)
            {
               this.var_81.image = _loc1_.data as BitmapData;
               this.var_81.imageCallback = _loc1_.id;
            }
         }
         if(this.var_81.image != null)
         {
            ...
            if(_loc2_)
            {
               ...
               _loc2_.bitmap.draw(this.var_81.image,new Matrix(1,0,0,1,(_loc2_.width - this.var_81.image.width) / 2,(_loc2_.height - this.var_81.image.height) / 2));
            }
         }
      }
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "buy_button":
               this._catalog.buyMarketPlaceOffer(this.var_81.offerId);
               this.hide();
               break;
      ...
      {
         if(this.var_81 && this.var_81.imageCallback == param1)
         {
            this.var_81.image = param2;
            this.setImage();
         }
      }
