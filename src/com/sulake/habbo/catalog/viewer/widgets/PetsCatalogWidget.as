   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function dispose() : void
      {
         ...
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc1_.avatarRenderManager.include(this);
      ...
      override public function init() : void
      {
         ...
         this.var_800 = this.getPetTypeIndexFromProduct(_loc5_.localizationId);
         this.var_297 = [];
         this.var_545 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc9_ = _loc8_.avatarRenderManager.petDataManager.getPetData(this.var_800,this);
               if(_loc9_)
               {
                  this.var_545 = _loc9_.sellableBreeds;
                  this.var_297 = _loc9_.colors;
         ...
         var _loc6_:int = 0;
         while(_loc6_ < this.var_545.length)
         {
            _loc10_ = this.var_545[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(this.getRaceLocalizationKey(this.var_800,_loc10_.id),this.getRaceLocalizationKey(this.var_800,_loc10_.id)));
            _loc6_++;
         }
         this._offers.add(this.var_800,_loc5_);
      ...
      private function onWidgetsInitialized(param1:CatalogWidgetsInitializedEvent = null) : void
      {
         ...
         var _loc3_:* = [];
         for each(_loc4_ in this.var_297)
         {
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_444 = param1.index;
         if(this.var_444 < 0 || this.var_444 > this.var_297.length)
         {
            this.var_444 = 0;
         }
         this.updateImage();
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_444 >= 0 && this.var_444 < this.var_297.length)
         {
            _loc7_ = this.var_297[this.var_444] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = this.var_545[this.var_1532] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_800,_loc3_.id,_loc2_,AvatarScaleType.const_46,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_298,AvatarAction.const_648);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_444 >= this.var_297.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_297[this.var_444] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = this.var_545[this.var_1532] as IBreed;
