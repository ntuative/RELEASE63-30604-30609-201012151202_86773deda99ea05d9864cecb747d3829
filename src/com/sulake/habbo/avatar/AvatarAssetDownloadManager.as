package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_826:int = 3;
      
      private var var_377:AssetLoaderStruct;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_377 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_827 = new Timer(this.const_1838,1);
         this.var_827.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_377)
         {
            this.var_377.addEventListener(AssetLoaderEvent.const_30,this.onConfigurationComplete);
            this.var_377.addEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_377)
         {
            this.var_377.removeEventListener(AssetLoaderEvent.const_30,this.onConfigurationComplete);
            this.var_377.removeEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_826);
            if(this.var_1277.indexOf("?") > 0)
            {
               _loc2_ = this.var_1277 + "&retry=" + this.var_826;
            }
            else
            {
               _loc2_ = this.var_1277 + "?retry=" + this.var_826;
            ...
            this.var_377 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2299,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1026[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1026[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:AvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1609)
         {
            this.var_1025.push([param1,param2]);
            return;
         }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         {
            delete this.var_828[_loc3_];
         }
         while(_loc6_ < this.var_676.length)
         {
            _loc5_ = this.var_676[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_676.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_827.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_675.length > 0 && this.var_676.length < this.const_1286)
         {
            _loc1_ = this.var_675[0];
            _loc1_.startDownloading();
            this.var_676.push(this.var_675.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_675.indexOf(param1) == -1 && this.var_676.indexOf(param1) == -1)
         {
