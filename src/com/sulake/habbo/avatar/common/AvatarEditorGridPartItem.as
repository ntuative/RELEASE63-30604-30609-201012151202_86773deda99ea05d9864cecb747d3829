package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_517:Rectangle;
      
      private var var_1433:int = 0;
      
      private var var_354:IAvatarRenderManager;
      
      private var var_720:Boolean;
      
      private var var_1435:BitmapData;
      
      private const const_1809:int = 13421772;
      
      private const const_1808:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1433 = Math.max(this.var_1433,_loc6_.colorLayerIndex);
            }
         }
         this.var_354 = this.var_62.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_618 != null && !this.var_1434)
            {
               _loc3_ = this.var_618;
            }
            else
            ...
            if(this.var_625)
            {
               switch(this.var_625.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_51:
                     _loc2_.style = HabboIconType.const_613;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this._window.color = this.const_1809;
         }
         else
         {
            this._window.color = this.const_1808;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:AvatarFigureContainer = this.var_354.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_354.isFigureReady(_loc1_))
         {
            this.var_354.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_309[_loc6_] + "_" + FigureData.const_128;
               _loc4_ = this.var_354.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_517 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1435)
               {
                  ...
                  this.var_1435 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_517.width,this.var_517.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_517.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_517.y;
               if(this.var_1434 && _loc2_.colorLayerIndex > 0)
               {
