package com.sulake.habbo.communication.messages.outgoing.inventory.badges
{
   ...
   public class SetActivatedBadgesComposer implements IMessageComposer
   {
      ...
      private const const_1561:int = 5;
      ...
      {
         if(this.var_1090.length >= this.const_1561)
         {
            return;
         }
      ...
      public function getMessageArray() : Array
      {
         ...
         var _loc2_:int = 1;
         while(_loc2_ <= this.const_1561)
         {
            if(_loc2_ <= this.var_1090.length)
            {
               _loc1_.push(_loc2_);
               _loc1_.push(this.var_1090[_loc2_ - 1]);
            }
            else
            {
               _loc1_.push(_loc2_);
