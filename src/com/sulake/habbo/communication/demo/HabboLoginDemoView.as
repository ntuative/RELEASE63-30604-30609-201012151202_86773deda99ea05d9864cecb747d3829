package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      private var var_168:HabboCommunicationDemo;
      
      private var _window:IWindowContainer;
      
      private var var_167:SSOTicket;
      ...
      {
         this.var_168.windowManager.removeWindow("habbo_login_window");
      }
      ...
      {
         var _loc1_:XmlAsset = (this.var_168 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         this._window = this.var_168.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_128,HabboWindowStyle.const_37,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = this.var_168.windowManager.buildFromXML(_loc2_) as IWindowContainer;
         ...
         var _loc7_:SharedObject = SharedObject.getLocal(this.const_1562,"/");
         if(_loc5_ != null)
         {
         ...
         if(this.var_168.habboConfiguration)
         {
            if(this.var_168.habboConfiguration.getKey("use.sso","false") == "true")
            {
               _loc8_.select();
            }
         }
      ...
      public function populateUserList(param1:Map) : void
      {
         ...
         var _loc3_:XmlAsset = this.var_168.assets.getAssetByName("login_user_list_item") as XmlAsset;
         var _loc4_:IWindow = this.var_168.windowManager.buildFromXML(_loc3_.content as XML);
      ...
      private function windowEventProcessor(param1:WindowEvent = null, param2:IWindow = null) : void
      {
         ...
         if(_loc3_ != null)
         {
            ...
            _loc6_ = SharedObject.getLocal(this.const_1562,"/");
      ...
      private function listEventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_168.sendTryLogin(this.name,this.password,param2.id);
      }
      ...
      private function initSSOTicket() : void
      {
         ...
         {
            this.var_167.removeEventListener(SSOTicket.const_339,this.onParsedTicketSuccess);
            this.var_167.removeEventListener(SSOTicket.const_296,this.onParsedTicketFailure);
            this.var_167 = null;
         }
         var _loc1_:String = "";
         if(this.var_168.habboConfiguration != null)
         {
            _loc1_ = this.var_168.habboConfiguration.getKey("url.prefix",_loc1_);
            _loc1_ = _loc1_.replace("http://","");
            _loc1_ = _loc1_.replace("https://","");
         }
         var _loc2_:IHabboWebLogin = this.var_168.habboCommunication.habboWebLogin(this.name,this.password);
         this.var_167 = new SSOTicket(this.var_168.assets,_loc2_,_loc1_);
         this.var_167.addEventListener(SSOTicket.const_339,this.onParsedTicketSuccess);
         this.var_167.addEventListener(SSOTicket.const_296,this.onParsedTicketFailure);
      }
      
      private function onParsedTicketSuccess(param1:Event) : void
      {
         Logger.log("Got ticket: " + this.var_167.ticket);
         this.var_168.shockwaveClientUrl = this.var_167.shockwaveClientUrl;
         this.var_168.ssoTicket = this.var_167.ticket;
         this.var_167.removeEventListener(SSOTicket.const_339,this.onParsedTicketSuccess);
         this.var_167.removeEventListener(SSOTicket.const_296,this.onParsedTicketFailure);
         this.var_167 = null;
         dispatchEvent(new Event(const_443));
