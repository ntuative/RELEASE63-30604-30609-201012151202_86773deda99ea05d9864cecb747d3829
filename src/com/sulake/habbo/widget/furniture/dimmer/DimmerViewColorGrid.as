package com.sulake.habbo.widget.furniture.dimmer
{
   ...
   public class DimmerViewColorGrid
   {
      ...
      private var var_434:IItemGridWindow;
      
      private var _view:DimmerView;
      
      private var var_1484:XML;
      ...
      public function setSelectedColorIndex(param1:int) : void
      {
         ...
         if(param1 < 0 || param1 >= this.var_434.numGridItems)
         {
            return;
         }
         this.select(this.var_434.getGridItemAt(param1) as IWindowContainer);
      }
      
      private function populate(param1:IHabboWindowManager) : void
      {
         if(this._view == null || this.var_434 == null)
         {
            return;
         }
      ...
      private function select(param1:IWindowContainer) : void
      {
         ...
         {
            _loc2_ = this.var_296.getChildByName("chosen");
            if(_loc2_ != null)
            {
      ...
      private function populateColourGrid(param1:IHabboWindowManager) : void
      {
         ...
         var _loc13_:* = null;
         var _loc14_:* = null;
         this.var_434.destroyGridItems();
         this.var_296 = null;
         for each(_loc2_ in this.colors)
         {
            _loc3_ = param1.buildFromXML(this.var_1484) as IWindowContainer;
            ...
            _loc3_.width = this.var_532.width;
            _loc3_.height = this.var_532.height;
            this.var_434.addGridItem(_loc3_);
            _loc4_ = _loc3_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
            if(_loc4_ != null)
            {
               _loc4_.bitmap = new BitmapData(this.var_532.width,this.var_532.height,true,0);
               _loc4_.bitmap.copyPixels(this.var_532,this.var_532.rect,new Point(0,0));
            }
            _loc5_ = _loc3_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
            if(_loc5_ != null)
            {
               _loc5_.bitmap = new BitmapData(this.var_963.width,this.var_963.height,true,0);
            ...
            if(_loc6_ != null)
            {
               _loc6_.bitmap = new BitmapData(this.var_785.width,this.var_785.height,true,16777215);
               _loc6_.bitmap.copyPixels(this.var_785,this.var_785.rect,new Point(0,0),null,null,true);
      ...
      {
         var _loc2_:int = this.var_434.getGridItemIndex(param1.target as IWindow);
